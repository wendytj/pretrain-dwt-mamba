...
def train_one_epoch(model, dataloader, criterion, optimizer, device, gpu_transform, accumulation_steps=1):
    model.train()
    running_loss = 0.0
    correct = 0
    total = 0
...
def main():
    ...
    for epoch in range(1, CONFIG["max_epochs"] + 1):
        train_loss, train_acc = train_one_epoch(model, train_loader, criterion, optimizer, device, gpu_train_transform, accumulation_steps=CONFIG["accumulation_steps"])
        val_loss, val_acc, val_eval = evaluate(model, val_loader, criterion, device, gpu_eval_transform)
    ...
    logger.save_best_results_json(
        train_eval=train_eval_best, # type: ignore
        val_eval=best_eval_tuples_val, # type: ignore
        test_eval=test_eval
    )
